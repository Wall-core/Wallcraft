    ...
    if (!m_canTrigger)                // Exceptions (some periodic triggers)
    {
        switch (m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_PRIEST:
                // Touch of Weakness / Devouring Plague / Wallcraft Penance
                if (m_spellInfo->IsFitToFamilyMask<CF_PRIEST_TOUCH_OF_WEAKNESS, CF_PRIEST_DEVOURING_PLAGUE, CF_PRIEST_FLASH_HEAL>())
                    m_canTrigger = true;
                break;
            default:
                break;
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (m_caster->IsPlayer())
    {
        ...
        switch (m_spellInfo->Id)
        {
            // Mongoose Bite						-- WALLCRAFT CHANGES
/*          case 1495:
            case 14269:
            case 14270:
            case 14271:
    ...
    for (uint8 i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        switch (m_spellInfo->Effect[i])
        {
            case SPELL_EFFECT_DUMMY:
            {
                if (m_spellInfo->SpellIconID == 156 || m_spellInfo->SpellIconID == 3013) // Holy Shock & Wallcraft Penance
                {
