...
void Aura::HandleAuraDummy(bool apply, bool Real)
{
    ...
    if (apply)
    {
        switch (GetSpellProto()->SpellFamilyName)
        {
			...
			case SPELLFAMILY_WARRIOR:
			{
				if (GetSpellProto()->SpellIconID == 1741)
				{
					Unit* caster = GetCaster();
					if (!caster) break;
					caster->SetUnitMovementFlags(MOVEFLAG_NONE);
				}
				break;
			}
        }
    ...
    else
    {
        ...
        switch (GetId())
        {
			...
			case 35114: // Wallcraft - Spirit of the Astral Wolf
		...
		switch (GetSpellProto()->SpellFamilyName) // Wallcraft - Seed of Life
		{
			case SPELLFAMILY_DRUID:
			{
				Unit* caster = GetCaster();
				Unit* target = GetTarget();
				ObjectGuid casterGUID = GetCasterGuid();
				if (GetSpellProto()->IsFitToFamilyMask(UI64LIT(0x0000000000000050)) && m_removeMode == AURA_REMOVE_BY_DISPEL)
				{
					if (!caster->HasAuraType(SPELL_AURA_OVERRIDE_CLASS_SCRIPTS))
						return;
					Unit::AuraList const& aurasOverrideClassScripts = caster->GetAurasByType(SPELL_AURA_OVERRIDE_CLASS_SCRIPTS);
					for (const auto aurasOverrideClassScript : aurasOverrideClassScripts)
					{
						...
						switch (aurasOverrideClassScript->GetModifier()->m_miscvalue)
						{
							case 10000:
								if (urand(1,5))
								target->CastSpell(target, 35144, true, nullptr, this, casterGUID);
								break;
							case 10001:
								if (urand(2,5))
								target->CastSpell(target, 35144, true, nullptr, this, casterGUID);
				...
				if (GetSpellProto()->Id == 35144 && m_removeMode == AURA_REMOVE_BY_EXPIRE)			// Wallcraft - Seed of Life proc
				{
					int32 targethp = (target->GetMaxHealth() / 10);
					int32 charges = GetHolder()->GetStackAmount();
					int32 heal = (targethp * charges);
					if (m_removeMode == AURA_REMOVE_BY_EXPIRE)
						target->CastCustomSpell(target, 35145, heal, {}, {}, true, nullptr, this, casterGUID);
				}
				break;
			}
		}
...
std::pair<unsigned int, float> GetShapeshiftDisplayInfo(ShapeshiftForm form, Unit* target){
    ...
    case FORM_TRAVEL:
		...
		mod = 0.80f;
		break;
	case FORM_SWIFT_TRAVEL:				// Wallcraft Swift Travel Form
		if (target->IsPlayer())
		{
			if (Player::TeamForRace(target->GetRace()) == ALLIANCE)
				display_id = 20000;
...
void Aura::HandleAuraModShapeshift(bool apply, bool Real)
{
    ...
    switch (form)
    {
        ...
        case FORM_MOONKIN:
        {
			...
			// Wallcraft - Furor
			if (!apply && GetSpellProto()->HasAura(SPELL_AURA_MOD_SHAPESHIFT))
				target->CastSpell(target, 35126, true);
            break;
        }
        case FORM_BERSERKERSTANCE:
        {
    ...
    if (display_id > 0 && !target->GetTransForm())
    {
        ...
        {
			if (!target->HasAuraType(SPELL_AURA_MOD_SHAPESHIFT))	// Wallcraft - SwiftShift change
			{
    ...
    if (apply)
    {
        ...
        if (PowerType != POWER_MANA)
        {
            ...
            switch (form)
            {
/*              case FORM_CAT:			// Wallcraft Furor change
                case FORM_BEAR:
                case FORM_DIREBEAR:
                {
...
void Aura::HandleChannelDeathItem(bool apply, bool Real)
{
    if (Real && !apply)
    {
        ...
        // Soul Shard (target req.) --- WALLCRAFT CHANGE
...
float Aura::CalculateDotDamage() const
{
    ...
    switch (spellProto->SpellFamilyName)
    {
        case SPELLFAMILY_DRUID:
        {
			...
			{
				if (caster->GetTypeId() == TYPEID_PLAYER)
					damage += caster->GetTotalAttackPowerValue(BASE_ATTACK) / 100;
			}
            break;
        }
		case SPELLFAMILY_WARRIOR: // Wallcraft Rend update
		{
...
void Aura::HandleShapeshiftBoosts(bool apply)
{
    ...
    if (apply)
    {
        ...
        if (target->GetTypeId() == TYPEID_PLAYER)
        {
			...
			// Wallcraft - Moonkin Aura
			if (((Player*)target)->HasSpell(24858))
			{
				SpellEntry const* spellInfo = sSpellMgr.GetSpellEntry(24907);
				if (spellInfo && spellInfo->Stances & (1 << (form - 1)))
					target->CastSpell(target, 24907, true, nullptr, this);
			}

			// Wallcraft - Aura of Life
			if (((Player*)target)->HasSpell(775))
			{
				SpellEntry const* spellInfo = sSpellMgr.GetSpellEntry(35134);
				if (spellInfo && spellInfo->Stances & (1 << (form - 1)))
					target->CastSpell(target, 35134, true, nullptr, this);
			}
...
void SpellAuraHolder::_AddSpellAuraHolder()
{
	...
	// Wallcraft - Armor of the Righteous trigger
	if (GetSpellProto()->IsFitToFamilyMask<CF_PALADIN_DEVOTION_AURA>())
		m_target->ModifyAuraState(AURA_STATE_DEVOTED, true);

	// Wallcraft - Righteous Fury trigger
	if (GetSpellProto()->IsFitToFamilyMask<CF_PALADIN_RIGHTEOUS_FURY>())
		m_target->ModifyAuraState(AURA_STATE_RIGHTEOUS, true);
...
void SpellAuraHolder::_RemoveSpellAuraHolder()
{
    ...
    if (m_removeMode != AURA_REMOVE_BY_DELETE)
    {
        ...
        {
            case SPELLFAMILY_PALADIN:
				// Paladin Judgement trigger removal
                if (m_spellProto->IsSealSpell())
					removeState = AURA_STATE_JUDGEMENT;     // Update Seals information
				// Wallcraft - Armor of the Righteous trigger removal
				if (m_spellProto->IsFitToFamilyMask<CF_PALADIN_DEVOTION_AURA>())
					removeState = AURA_STATE_DEVOTED;
                break;
        }
		...
		if (removeState || m_spellProto->HasAttribute(SPELL_ATTR_EX_PREVENTS_ANIM))
		{
			...
			if (removeState && !foundAuraWithSameAuraState)
			{
				...
				if (m_spellProto->IsFitToFamilyMask<CF_PALADIN_RIGHTEOUS_FURY>()) // Wallcraft Righteous Fury trigger removal
					m_target->ModifyAuraState(AuraState(AURA_STATE_RIGHTEOUS), false);
			}
			if (m_spellProto->HasAttribute(SPELL_ATTR_EX_PREVENTS_ANIM) && !foundAuraWithPreventsAnimAttribute)
...
void SpellAuraHolder::CalculateForDebuffLimit()
{
    ...
    {
        m_debuffLimitScore = 4;		 // Wallcraft change
        return;
    }
