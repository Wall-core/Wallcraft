...
pEffect SpellEffects[TOTAL_SPELL_EFFECTS] =
{
	...
	&Spell::EffectUnused,									//132 SPELL_EFFECT_APPLY_AREA_AURA_RAID
	&Spell::EffectUnused,									//133 SPELL_EFFECT_APPLY_AREA_AURA_OWNER
	&Spell::EffectFeralLeap,								//134 SPELL_EFFECT_FERAL_LEAP				Wallcraft Feral Leap effect - Implemented in Unit::FeralLeap
};
    ...
    if (unitTarget && unitTarget->IsAlive())
    {
        switch (m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_WARRIOR:
            {
                ...
                // Shield Slam - Wallcraft change
                else if (m_spellInfo->IsFitToFamilyMask<CF_WARRIOR_SHIELD_SLAM>())
				//  damage += m_casterUnit->GetShieldBlockValue();
					damage += ((m_casterUnit->GetPower(POWER_RAGE) * 0.1) * (m_casterUnit->GetShieldBlockValue() * 0.05));
                // Execute trigger
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
				...
				case 35263:
				{
					if (!unitTarget)
						return;
					unitTarget->PlayDirectSound(10007);
					return;
				}
				...
				case 35126:									// Wallcraft - Furor trigger
				{
					if (m_casterUnit)
					{
						// Check to see if the player has Furor talented
						Unit::AuraList const& auraDummy = m_casterUnit->GetAurasByType(SPELL_AURA_ADD_FLAT_MODIFIER);
						for (const auto itr : auraDummy)
						{
							if (itr->GetSpellProto()->SpellFamilyName == SPELLFAMILY_DRUID && itr->GetSpellProto()->SpellIconID == 238)
							{
								switch (m_casterUnit->GetShapeshiftForm())
								{
								case FORM_CAT:
									m_casterUnit->CastSpell(m_casterUnit, 17057, true, nullptr); // When switching from cat to bear, gain rage
									return;
								case FORM_BEAR:
								case FORM_DIREBEAR:
									m_casterUnit->CastSpell(m_casterUnit, 17099, true, nullptr); // When switching from bear to cat, gain energy
									return;
								}
							}
						}
                ...
                case 25938:                                 // Viscidus explode
                {
                    ...
                    // Fake death
                    //m_casterUnit->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_FLEEING_DEATH);
        ...
        case SPELLFAMILY_MAGE:
        {
            switch (m_spellInfo->Id)
            {
				...
				case 35099: // Wallcraft - Time Warp cooldown refresh
				{
					uint32 spell_id = m_currentBasePoints[eff_idx];
					auto cdCheck = [spell_id](SpellEntry const & spellEntry) -> bool { return (spellEntry.IsFitToFamily<SPELLFAMILY_MAGE, CF_MAGE_BLINK>() && spellEntry.GetRecoveryTime() > 0); };
					static_cast<Player*>(m_caster)->RemoveSomeCooldown(cdCheck);
					return;
				}
            }
        ...
        case SPELLFAMILY_WARRIOR:
        {
			// Wallcraft Concussion Blow immunity check
			if (m_spellInfo->Id == 12809)
			{
				m_caster->CastSpell(unitTarget, 35219, true);
				return;
			}
            // Execute
			...
			// Wallcraft Timed Block refresh
//			if (m_spellInfo->Id == 35071)
//			{
//				uint32 spell_id = m_currentBasePoints[eff_idx];
//				auto cdCheck = [spell_id](SpellEntry const & spellEntry) -> bool { return (spellEntry.IsFitToFamily<SPELLFAMILY_WARRIOR, CF_WARRIOR_REVENGE>() && spellEntry.GetRecoveryTime() > 0); };
//				static_cast<Player*>(m_caster)->RemoveSomeCooldown(cdCheck);
//				return;
//			}
            break;
			...
			switch (m_spellInfo->SpellIconID)
			{
				case 3013:                                   // Wallcraft Penance
				{
					...
					if (m_caster->IsFriendlyTo(unitTarget))
						m_caster->CastSpell(unitTarget, heal, true, nullptr, nullptr, m_originalCasterGUID, m_triggeredBySpellInfo, m_triggeredBySpellInfo);
					else
						m_caster->CastSpell(unitTarget, hurt, true, nullptr, nullptr, m_originalCasterGUID, m_triggeredBySpellInfo, m_triggeredBySpellInfo);
        ...
        case SPELLFAMILY_HUNTER:
        {
            switch (m_spellInfo->Id)
            {
				...
				case 35176:									// Wallcraft Viper Sting
				{
					m_caster->CastCustomSpell(m_originalCaster, 35177, m_triggeredByAuraBasePoints, {}, {}, true, nullptr, nullptr, m_originalCasterGUID);
					m_caster->CastCustomSpell(unitTarget, 35178, m_triggeredByAuraBasePoints, {}, {}, true, nullptr, nullptr, m_originalCasterGUID);						
					return;
				}
				break;
			}
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            switch (m_spellInfo->SpellIconID)
            {
				...
				// Wallcraft updates below
				case 206:													// Judgement of Wisdom cooldown refresh
				{
					if (!unitTarget)
						return;

					uint32 spell_id = m_currentBasePoints[eff_idx];
					auto cdCheck = [spell_id](SpellEntry const & spellEntry) -> bool { return (spellEntry.IsFitToFamily<SPELLFAMILY_PALADIN, CF_PALADIN_SEAL_OF_WISDOM_LIGHT>() && spellEntry.GetRecoveryTime() > 0); };
					static_cast<Player*>(m_caster)->RemoveSomeCooldown(cdCheck);
					return;
				}
				case 301:													// Judgement of Fury cooldown refresh
				{
					if (!unitTarget)
						return;

					uint32 spell_id = m_currentBasePoints[eff_idx];
					auto cdCheck = [spell_id](SpellEntry const & spellEntry) -> bool { return (spellEntry.IsFitToFamily<SPELLFAMILY_PALADIN, CF_PALADIN_RIGHTEOUS_FURY>() && spellEntry.GetRecoveryTime() > 0); };
					static_cast<Player*>(m_caster)->RemoveSomeCooldown(cdCheck);
					return;
				}
				case 304:													// Judgement of Righteousness cooldown refresh
				{
					if (!unitTarget)
						return;

					uint32 spell_id = m_currentBasePoints[eff_idx];
					auto cdCheck = [spell_id](SpellEntry const & spellEntry) -> bool { return (spellEntry.IsFitToFamily<SPELLFAMILY_PALADIN, CF_PALADIN_UNK32>() && spellEntry.GetRecoveryTime() > 0); };
					static_cast<Player*>(m_caster)->RemoveSomeCooldown(cdCheck);
					return;
				}
				case 305:													// Judgement of Salvation cooldown refresh
				{
					if (!unitTarget)
						return;

					uint32 spell_id = m_currentBasePoints[eff_idx];
					auto cdCheck = [spell_id](SpellEntry const & spellEntry) -> bool { return (spellEntry.IsFitToFamily<SPELLFAMILY_PALADIN, CF_PALADIN_BLESSING_OF_SALVATION1>() && spellEntry.GetRecoveryTime() > 0); };
					static_cast<Player*>(m_caster)->RemoveSomeCooldown(cdCheck);
					return;
				}
				case 307:													// Judgement of Justice cooldown refresh
				{
					if (!unitTarget)
						return;

					uint32 spell_id = m_currentBasePoints[eff_idx];
					auto cdCheck = [spell_id](SpellEntry const & spellEntry) -> bool { return (spellEntry.IsFitToFamily<SPELLFAMILY_PALADIN, CF_PALADIN_UNK22>() && spellEntry.GetRecoveryTime() > 0); };
					static_cast<Player*>(m_caster)->RemoveSomeCooldown(cdCheck);
					return;
				}
                case 561:													// Judgement of Command cooldown refresh
                {
                    if (!unitTarget)
                        return;

					uint32 spell_id = m_currentBasePoints[eff_idx];
					auto cdCheck = [spell_id](SpellEntry const & spellEntry) -> bool { return (spellEntry.IsFitToFamily<SPELLFAMILY_PALADIN, CF_PALADIN_SEAL_OF_COMMAND>() && spellEntry.GetRecoveryTime() > 0); };
					static_cast<Player*>(m_caster)->RemoveSomeCooldown(cdCheck);
		...
		case SPELLFAMILY_ROGUE: // Wallcraft Ghostly Strike
		{
			if (m_spellInfo->IsFitToFamilyMask<CF_ROGUE_GHOSTLY_STRIKE>())
			{
				uint32 spell_id = m_currentBasePoints[eff_idx];
				auto cdCheck = [spell_id](SpellEntry const & spellEntry) -> bool { return (spellEntry.IsFitToFamily<SPELLFAMILY_ROGUE, CF_ROGUE_GHOSTLY_STRIKE>() && spellEntry.GetRecoveryTime() > 0); };
				static_cast<Player*>(m_caster)->RemoveSomeCooldown(cdCheck);
				return;
			}
			break;
...
void Spell::EffectHeal(SpellEffectIndex eff_idx)
{
    if (unitTarget && unitTarget->IsAlive() && damage >= 0)
    {
        ...
        if (m_spellInfo->Id == 18562)
        {
            ...
            unitTarget->RemoveAurasDueToSpell(targetAura->GetId(), nullptr, AURA_REMOVE_BY_DISPEL); // Wallcraft change
...
void Spell::EffectDispel(SpellEffectIndex eff_idx)
{
    ...
    // Shield Slam 50% chance dispel // Wallcraft change
        ...
        if (Creature* summon = m_caster->SummonCreature(creature_entry, px, py, pz, m_caster->GetOrientation(), summonType, duration))
        {
            ...
            switch (m_spellInfo->Id)
            {
				...
				case 35259:
					if (m_casterUnit)
					{
						uint32 textId = PickRandomValue(100005, 100006, 100007, 100008, 100009);
						summon->MonsterWhisper(textId, m_casterUnit);
					}
					break;
            }
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_PALADIN:
        {
			...
			else if (m_spellInfo->IsFitToFamilyMask<CF_PALADIN_JUDGEMENT>())
			{
				...
				while (m_casterUnit->HasAuraState(AURA_STATE_JUDGEMENT)) // Wallcraft Alpha Paladin change
				{
        ...
        case SPELLFAMILY_SHAMAN:
        {
            switch(m_spellInfo->Id)
            {
				...
				case 35115:		// Wallcraft -  Astral Step
				{
					if (!m_casterUnit)
						return;
					if (m_casterUnit->GetTypeId() != TYPEID_PLAYER)
						unitTarget->SetStandState(UNIT_STAND_STATE_STAND);
						m_casterUnit->SetVisibility(VISIBILITY_RESPAWN);
...
void Spell::EffectFeralLeap(SpellEffectIndex eff_idx)
{
	if (!unitTarget || unitTarget->IsTaxiFlying())
		return;

	unitTarget->FeralLeap(m_caster, float(m_spellInfo->EffectMiscValue[eff_idx]) / 10, damage / 10);
}
...
void Spell::EffectPlayerPull(SpellEffectIndex eff_idx)
{
	...
	case 35077: // Wallcraft - Trapper's Net
    {
...
void Spell::EffectDestroyAllTotems(SpellEffectIndex /*eff_idx*/)
{
	...
	if (mana)
		m_casterUnit->CastCustomSpell(m_casterUnit, 35193, mana, {}, {}, true, nullptr);
}
