...
bool Unit::IsTriggeredAtSpellProcEvent(Unit* pVictim, SpellAuraHolder* holder, SpellEntry const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, SpellProcEventEntry const*& spellProcEvent, bool dontTriggerSpecial) const
{
    ...
    if (procSpell)
    {
...
#endif
        {
			return procFlag & PROC_FLAG_TAKE_HARMFUL_SPELL;
        }
#endif
		...
		// PALADIN
		// Wallcraft - Seal of Command
		if (spellProto->SpellIconID == 561 && spellProto->SpellVisual == 5622)
		{
    ...
    // In most cases req get honor or XP from kill -- WALLCRAFT CHANGE
    ...
    switch (dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            switch (dummySpell->Id)
            {
				...
				{
					basepoints[0] = dither(triggerAmount * damage / 100);
					this->CastCustomSpell(this, 35031, basepoints[0], {}, {}, true, castItem, triggeredByAura);
					return SPELL_AURA_PROC_OK;                                // no hidden cooldown
				}
...
SpellAuraProcResult Unit::HandleProcTriggerSpellAuraProc(Unit* pVictim, uint32 damage, Aura* triggeredByAura, SpellEntry const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    switch (auraSpellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_MAGE:
            ...
            break;
        case SPELLFAMILY_WARRIOR:
			if (auraSpellInfo->SpellIconID == 1474)
			{
				uint32 cleave = auraSpellInfo->EffectBasePoints[0];
				basepoints[0] = cleave + 1;
				trigger_spell_id = 35220;
			}
			if (auraSpellInfo->SpellIconID == 215)
			{
				uint32 multiplier = auraSpellInfo->EffectBasePoints[0];
				uint32 maxhealth = GetMaxHealth();
				basepoints[0] =  multiplier * maxhealth / 100;
				trigger_spell_id = 35195;
			}
            break;
        case SPELLFAMILY_WARLOCK:
        {
        ...
        case SPELLFAMILY_PRIEST:
        {
			...
			else if (auraSpellInfo->SpellIconID == 130)					// Wallcraft - Feedback
			{
				if (IsAlive() || pVictim->IsAlive())
					CastCustomSpell(pVictim, 35158, triggerAmount, {}, {}, true, nullptr, triggeredByAura);
				if (pVictim->GetPowerType() == POWER_MANA)
					CastCustomSpell(pVictim, 35159, triggerAmount, {}, {}, true, nullptr, triggeredByAura);
				return SPELL_AURA_PROC_OK;	
			}
			else if (auraSpellInfo->SpellIconID == 3014)                 // Wallcraft - Divine Aegis
			{
				target = pVictim;
				switch (auraSpellInfo->Id)
				{
					case 35161:
						basepoints[0] = damage * 5 / 100;
						trigger_spell_id = 35160;
						break; // Rank 1
					case 35162:
						basepoints[0] = damage * 10 / 100;
						trigger_spell_id = 35160;
						break; // Rank 2
					case 35163:
						basepoints[0] = damage * 15 / 100;
...
#endif
			if (auraSpellInfo->SpellIconID == 3016)		//	Wallcraft - Creature Comforts
			{
					trigger_spell_id = 35188;
        ...
        case SPELLFAMILY_PALADIN:
        {
			...
			// Wallcraft Judgement of Betrayal
			else if (auraSpellInfo->IsFitToFamilyMask<CF_PALADIN_JUDGEMENT_OF_COMMAND>())
			{
				switch (auraSpellInfo->Id)
				{
					// Correspond the Judgements to the Seal
				case 35036:
					trigger_spell_id = 20375;
					break; // Rank 1
				case 35053:
					trigger_spell_id = 20915;
					break; // Rank 2
				case 35054:
					trigger_spell_id = 20918;
					break; // Rank 3
				case 35055:
					trigger_spell_id = 20919;
					break; // Rank 4
				case 35056:
					trigger_spell_id = 20920;
					break; // Rank 5
				default:
					sLog.outError("Unit::HandleProcTriggerSpell: Spell %u does not correspond to any Seal of Command", auraSpellInfo->Id);
					return SPELL_AURA_PROC_FAILED;
				}
				pVictim->AddAura(trigger_spell_id, ADD_AURA_POSITIVE, this);
				return SPELL_AURA_PROC_OK;
			}
            // Illumination
