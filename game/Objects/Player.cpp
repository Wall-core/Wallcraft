...
Creature* Player::SummonPossessedMinion(uint32 creatureId, uint32 spellId, float x, float y, float z, float ang)
{
    ...
    Creature* pCreature = SummonCreature(creatureId, x, y, z, ang, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 0, false, 0, nullptr, GetTransport());
...
uint32 Player::GetResurrectionSpellId() const
{
    ...
    for (const auto dummyAura : dummyAuras)
    {
		...
		// Wallcraft Reincarnation Totem					// prio: 3 (max, non death persistent)
		else if (dummyAura->GetId() == 35097)
		{
			prio = 3;
			spellId = 35098;
		}
