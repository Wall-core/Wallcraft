...
void Unit::Update(uint32 update_diff, uint32 p_time)
{
	...
	if (IsAlive())
	{
		...
		ModifyAuraState(AURA_STATE_POWERLESS_20_PERCENT, GetPower(Powers()) < GetMaxPower(Powers()) * 0.20f);			// Wallcraft Power based triggers
		ModifyAuraState(AURA_STATE_HEALTHLESS_35_PERCENT, GetHealth() < GetMaxHealth() * 0.35f);						// Wallcraft Ardent Defender
		ModifyAuraState(AURA_STATE_HEALTHLESS_100_PERCENT, GetHealth() < GetMaxHealth());								// Wallcraft Cause for Concern
		ModifyAuraState(AURA_STATE_POWERLESS_100_PERCENT, GetPower(Powers()) < GetMaxPower(Powers()));					// Wallcraft Interloper
		ModifyAuraState(AURA_STATE_NOT_MOVING, !IsMoving());															// Wallcraft Movement based triggers
		if (IsPlayer())																									//
			ModifyAuraState(AURA_STATE_BAD_MOON, (HasAuraState(AURA_STATE_POWERLESS_20_PERCENT) && HasAura(24858)));	// Wallcraft Bad Moon
			ModifyAuraState(AURA_STATE_DEEP_ROOTS, (HasAuraState(AURA_STATE_NOT_MOVING) && HasAura(775)));				// Wallcraft Deep Roots
			ModifyAuraState(AURA_STATE_NOT_FALLING, !HasUnitMovementFlag(MOVEFLAG_JUMPING));							// Wallcraft Feral Leap
	}
...
bool Unit::RemoveNoStackAurasDueToAuraHolder(SpellAuraHolder* holder)
{
    ...
    for (i = m_spellAuraHolders.begin(); i != m_spellAuraHolders.end(); i = next)
    {
        ...
        if (i_spellId == spellId)
        {
			...
			case 15286:		 // Wallcraft Vampiric Embrace change
            case 19977:
            case 19978:
            case 19979:
            case 25890:
...
float Unit::SpellHealingBonusTaken(SpellCaster* pCaster, SpellEntry const* spellProto, SpellEffectIndex effectIndex, float healamount, DamageEffectType damagetype, uint32 stack, Spell* spell) const
{
    ...
    // Taken mods
    // Healing Wave cast
    if (spellProto->IsFitToFamily<SPELLFAMILY_SHAMAN, CF_SHAMAN_HEALING_WAVE>() || spellProto->IsFitToFamily<SPELLFAMILY_SHAMAN, CF_SHAMAN_CHAIN_HEAL>())
    {
...
float Unit::GetRegenMPPerSpirit() const
{
    ...
    {
        case CLASS_DRUID:
            addvalue = (Spirit / 4 + 12.5); // Wallcraft value change
            break;
        case CLASS_HUNTER:
...
void Unit::SetDeathState(DeathState s)
{
    ...
    if (s == JUST_DIED)
    {
		...
		// Wallcraft - Custom Aurastate cleanups
		ModifyAuraState(AURA_STATE_DEVOTED, false);
		ModifyAuraState(AURA_STATE_POWERLESS_100_PERCENT, false);
		ModifyAuraState(AURA_STATE_POWERLESS_20_PERCENT, false);
		ModifyAuraState(AURA_STATE_HEALTHLESS_100_PERCENT, false);
		ModifyAuraState(AURA_STATE_HEALTHLESS_35_PERCENT, false);
		ModifyAuraState(AURA_STATE_NOT_FALLING, false);
		ModifyAuraState(AURA_STATE_NOT_MOVING, false);
		ModifyAuraState(AURA_STATE_BAD_MOON, false);
		ModifyAuraState(AURA_STATE_DEEP_ROOTS, false);
		ModifyAuraState(AURA_STATE_RIGHTEOUS, false);
		//
...
{
	if (HasAura(35011)) // Wallcraft - Judgement of Wickedness
...
void Unit::KnockBack(float angle, float horizontalSpeed, float verticalSpeed)
{
	...
	InterruptSpellsWithInterruptFlags(SPELL_INTERRUPT_FLAG_MOVEMENT);		// Wallcraft change - fixes interrupts
...
void Unit::FeralLeap(WorldObject* target, float horizontalSpeed, float verticalSpeed) // Wallcraft Feral Leap
{
	if (IsRooted())
		return;
...
float Unit::GetCombatReach(Unit const* pVictim, bool ability, float flat_mod) const
{
	...
	if (this->HasAura(35073))	// Wallcraft - Primal Hunter
		reach += 5.0f;
