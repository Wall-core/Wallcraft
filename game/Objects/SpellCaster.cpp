...
void SpellCaster::DealSpellDamage(SpellNonMeleeDamage* damageInfo, bool durabilityLoss)
{
	...
	// Wallcraft expanded Judgement refresh
	if (damageInfo && SPELL_DIRECT_DAMAGE && spellProto->EquippedItemClass == ITEM_CLASS_WEAPON)
	{
		Unit::SpellAuraHolderMap const& vAuras = pVictim->GetSpellAuraHolderMap();
